...
def FindMatches(verbose=False):
    ...
    dend = shc.dendrogram(shc.linkage(locations, method='ward'))
...
def ReadCsv(block,src,Ch,file):
    ...
    # # print(data.columns)
    data.rename(columns={"Velocity Ch.%d (m/sec)" % src: "Velocity Ch. %d (m/sec)" % src},
                inplace=True)
    data.rename(columns={"Velocity Ch. %d (msec)" % src: "Velocity Ch. %d (m/sec)" % src},
                inplace=True)
    data.rename(columns={"Analog Extern Ch 2": "Analog Extern Ch. 2"},
                inplace=True)
    data.rename(columns={"AnalogExtern Time 2 (sec)": "Analog Extern Time 2 (sec)"},
                inplace=True)
    # print(data.columns)
...
def PlotVStatsPolar(block, plane, orientation, X, show=False):
    ...
    for k,src,lbl in list(zip(range(len(Src)),Src,Lbl)):
        ...
        figtitle = figtitle + '\n' + r'Plane %d | X=%.0f mm | Slot spacing=%0.2f$^\circ$ | Slot width=%0.2f$^\circ$ | Overlap=%.0f%%' % \
            (plane,X,settings['Step'],settings['Wslot'],overlap)
